...
// ===== normaliza categoria vinda do cardápio (mesmo se vier em outro campo) =====
...
export default function Checkout() {
  ...
  useEffect(() => {
    try {
      ...
      // garante category/categoria mesmo que o cardápio não tenha salvo
  ...
  // ===== CLASSIFICAÇÃO SOMENTE POR CATEGORY =====
  const norm = (s) => String(s || '').toLowerCase().trim();
  const catOf = (it) => norm(it?.category || it?.categoria || it?.cat || '');

  const isPizza = (it) => catOf(it).includes('pizza'); // pizza doce/salgada
  const isBorda = (it) => catOf(it).includes('borda'); // borda = apenas quando vier category = 'borda'
  ...
  // rótulo "5 - Vegetariana (G)" a partir do id
  ...
  const linhas = useMemo(() => {
    return items.map((it) => {
      ...
      return {
        descricao, // <- aqui vai "117 Borda Cheddar — 5 - Vegetariana (G)"
        qtd: it?.qtd || 1,
  ...
  // >>>>>>>>>> CONFIRMAR (envia para webhook via proxy) <<<<<<<<<<
...
${(comentarios || '').trim() ? `Comentário: ${comentarios.trim()}` : ''}`;
  ...
  // continua fluxo (leva pra assoc se faltar)
  ...
  return (
    <main className="container" style={{ maxWidth: 760, margin: '24px auto' }}>
          ...
          {items.map((it) => (
            ...
            >
              ...
              >
                ...
                {/* se for borda, mostra a associação no carrinho */}
                ...
                {isBorda(it) &&
                  Array.isArray(it.linkedToList) &&
                  it.linkedToList.length > 0 && (
                    <>
                      <span style={{ opacity: 0.5 }}>—</span>
                      <span style={{ fontWeight: 600 }}>
                        {it.linkedToList.filter(Boolean).map(pizzaLabelById).join(' , ')}
                      </span>
                    </>
                  )}
                        ...
                        onChange={(e) => {
                          ...
                          setItems((prev) =>
                            prev.map((it) => {
                              ...
                              if (q > 1) {
                                const list =
                                  it.linkedToList && Array.isArray(it.linkedToList)
                                    ? [...it.linkedToList]
                                    : Array(q).fill(null);
                                list[u.idx - 1] = pid;
      ...
      {/* TOTAIS */}
      <div
        style={{ display: 'flex', justifyContent: 'space-between', marginTop: 12, color: '#0f172a' }}
      >
        <div>Subtotal</div>
        <div>R$ {fmt(subtotal)}</div>
      </div>
      <div
        style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4, color: '#0f172a' }}
      >
