...
function makeHalfPending(half) {
  ...
  return {
    ...
    size: half.size || 'G',
   categoria: 'pizza', // <<< ADICIONE ESTA LINHA
    price,
    preco: price,
    qtd: 1,
    meta: { ...half.meta },
 ...
 function makeHalfCombo(a, b, priceRule = 'max') {
  ...
  return {
    ...
    qtd: 1,
    size,
   categoria: 'pizza', // ✅ AQUI está a correção certa
    halves: [
      { productId: a.productId, code: codeA, name: nameA, price: halfA , categoria: 'pizza'},
      { productId: b.productId, code: codeB, name: nameB, price: halfB , categoria: 'pizza'},
    ],
    name: pretty,
    price,
    preco: price,
