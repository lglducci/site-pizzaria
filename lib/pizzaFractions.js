...
function makeHalfPending(half) {
  ...
  const code = half.code ?? half.productId ?? '';
  const name = `Meia ${code ? code + ' ' : ''}${half.name} (1/2)`;
  return {
    id,
    type: 'half',
    isPending: true,
    ...
    code,
    name,                // já mostra “Meia 45 Confete (1/2)”
    size: half.size || 'G',
    price,
    preco: price,
    qtd: 1,
...
function makeHalfCombo(a, b, priceRule = 'max') {
  ...
  const codeA = a.code ?? a.productId ?? '';
  const codeB = b.code ?? b.productId ?? '';
  ...
  const pretty =
    `Meia ${codeA ? codeA + ' ' : ''}${a.name} (1/2) + ` +
    `Meia ${codeB ? codeB + ' ' : ''}${b.name} (1/2) (${size})`;
  ...
  return {
    ...
    size,
    halves: [
      { productId: a.productId, code: codeA, name: a.name, price: pA },
      { productId: b.productId, code: codeB, name: b.name, price: pB },
    ],
    name: pretty,      // <<< rótulo final exatamente como você quer
    price,
    preco: price,
    meta: {},
  };
...
export function addHalfToCart(cart, half, opts = {}) {
  ...
  if (idx >= 0) {
    ...
    const combo = makeHalfCombo(
      { productId: pending.productId, code: pending.code, name: pending.name.replace(/^Meia\s.*?\s/, '').replace(/\s\(1\/2\)$/, ''), price: pending.price, size: pending.size },
      half,
      priceRule
    );
